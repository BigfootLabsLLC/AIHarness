...
## Version 0.1 — "The Bridge" (HTTP Tool Server)

**Goal:** Built-in HTTP tool server that external AIs can connect to. AIHarness is the "tool provider" for your existing AI assistants.

**Why this first:** External AIs need a safe local bridge. We build a local HTTP server that **you** control, and any AI can call it.

**Definition of Done:**
- [ ] Tauri app launches (system tray or window)
- [ ] **Local HTTP Tool Server** running on localhost
  - [ ] HTTP endpoints for tool calls (localhost only)
...
> "External AIs can connect to AIHarness's built-in HTTP server and call tools to read project files and todo lists through the bridge."
...
curl -X POST http://127.0.0.1:8787/call \
  ...
  -d '{"name":"read_file","arguments":{"path":"/tmp/todo.txt"}}'
```
...
**Goal:** AIHarness gets its own AI integration. No external client needed — chat directly in the app.
